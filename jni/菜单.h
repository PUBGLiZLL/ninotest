...
{
    if (!STEPC || !Char || !State_Vehicle || !STEPC->VehicleUserComp)
        return;
...
bool IsValidPointer(T *ptr)
{
    ...
    // 基本地址范围检查
    if (addr < 0x10000 || addr > 0xFFFFFFFFFFFFFF00)
        return false;
...
SDK::FVector GetServerPlayerPositionSafe()
{
    ...
    try
    {
        ...
        if (!IsValidPointer(playerCtrl))
            return MakeZeroVector();

        // 检查是否是服务端
        if (!playerCtrl->HasAuthority())
            return MakeZeroVector();
...
void SafeTeleportAllPlayers()
{
    ...
    try
    {
        ...
        for (int i = 0; i < totalPlayers; i++)
        {
            ...
            if (!IsValidPointer(info.PlayerCharacter))
                continue;
...
void 初始化菜单()
{
    ...
    if (ImGui::Begin(("全自动服务端")), 0, ImGuiWindowFlags_NoSavedSettings)
    {
        ...
        ImGui::SetColumnOffset(1, 160);
        {
            ...
            ImGui::BeginGroup();
            if (ImGui::Button("开服区", ImVec2(145, 70)))
                currentPage = PAGE_MULTI;
